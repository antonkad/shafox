...
//
// Given a commit SHA, derive a fully reproducible identity: a codename, an
// emoji, and a palette + fox's traits. The same SHA ALWAYS yields the same
// codename and traits, on any machine, forever. That determinism is what makes
// two concurrently deployed commits instantly distinguishable — and what makes
// a rollback visibly snap back to a name you've seen before.
//
// This is deliberately NOT where the page-level accent comes from: the page
// accent is the fixed stage colour in vineyard.ts (c1 blue, c2 green, c3
// yellow), while the SHA-derived colours here and in `commitAccent` tag
// per-written-commit provenance.
